...
module.exports = (globalDevices) => {
	...
	const get_config_map = (firmware) => {
		...
		return {
			...
			"input_1_idle_state": {
				...
				"write_index": 33,
				"descriptions": {
					"title": "Set Input 1 Idle State",
					"main_caption": "If the input remains in this configured state for more than the Input 1 idle time, a report message is sent"
				},
				"default_value": 1,
				"validator": {
					"type": "uint8",
			...
			"input_2_idle_state": {
				...
				"html_id": "input_2_idle_state_133"
			},
			"input_1_idle_time": {
				"read_index": 46,
				"write_index": 35,
				"descriptions": {
					"title": "Set Input 1 Idle Time",
					"main_caption": "If the input state remains in the Input 1 idle state for more than this time, a report message is sent"
				},
				"default_value": 60,
				"validator": {
					"type": "uint16be",
				...
				"html_id": "input_idle_time_132"
			},
			"input_2_idle_time": {
				"read_index": 48,
				"write_index": 37,
				"descriptions": {
					"title": "Set Input 2 Idle Time",
					"main_caption": "If the input state remains in the Input 1 idle state for more than this time, a report message is sent"
				},
				"default_value": 60,
				"validator": {
	...
	const parse = (d, payload) => {
		// Isolate the lower 4 bits (0-3) for the base report type
		const baseType = d[17] & 0x0F;

		// Map the standard types using a simple array (matches 0, 1, 2, 3)
		const standardTypes = ["Regular", "Shift end", "Interrupt", "Threshold"];
		
		let report_type = standardTypes[baseType] || "Unknown";

		// Handle the 0x04 Input State logic
		if (baseType === 4) {
			const status = d[17] >> 4; // Shift right to look at bits 4 and 5
			...
			report_type = status > 0 
				? `${inputMap[status]}, time limit` 
				: "Unknown"; // Fallback if bits are 0
		}
	...
	return {
		type: 133,
		name: 'Two Channel Production Counter',
		parse,
