...
module.exports = (globalDevices, emitter) => {
	...
	const init_globalDevices_stream = (deviceAddr, payload, expected_packets, parsed, probe) => {
		...
		globalDevices[deviceAddr][probe].packet_stream_timeout = setTimeout(() => {
			...
			emitter.emit('sensor_data-520', parsed);
	...
	const parse = (payload, parsed, mac) => {
		...
		if (firmware > 5) {
			...
			if (mode == 'raw') { // Time-domain
				...
				return {
					mode: 'processed',
					current_1_ma: payload.slice(8, 12).reduce(msbLsb),
					frequency_1: payload.slice(12, 13).reduce(msbLsb),
					current_2_ma: payload.slice(14, 18).reduce(msbLsb),
					frequency_2: payload.slice(18, 20).reduce(msbLsb),
					current_3_ma: payload.slice(20, 24).reduce(msbLsb),
					frequency_3: payload.slice(24, 26).reduce(msbLsb),
					current_4_ma: payload.slice(26, 30).reduce(msbLsb),
					frequency_4: payload.slice(30, 32).reduce(msbLsb),
					current_5_ma: payload.slice(32, 36).reduce(msbLsb),
					frequency_5: payload.slice(36, 38).reduce(msbLsb),
					current_6_ma: payload.slice(38, 42).reduce(msbLsb),
					frequency_6: payload.slice(42, 44).reduce(msbLsb),
					humidity: payload.slice(44, 46).reduce(msbLsb) / 100,
					temperature: signInt(payload.slice(46, 48).reduce(msbLsb), 16) / 100
				};
			}
		} else {
			return {
