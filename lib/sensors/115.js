	...
	const parse = (payload, parsed, mac) => {
		...
		if (vibration_mode === 0) { // basic mode
			return {
				...
				rms_mV: parseFloat((parsed.slice(10, 12).reduce(msbLsb) / 100).toFixed(3)),
				rms_ultrasound_dBuV: parsed.slice(12, 14).reduce(msbLsb)
			};
		}
		else { // detailed mode
			return {
				...
				rms_mV: parseFloat((parsed.slice(10, 12).reduce(msbLsb) / 100).toFixed(3)),
				rms_ultrasound_dBuV: parsed.slice(12, 14).reduce(msbLsb),
				peak_mV: parsed.slice(14, 16).reduce(msbLsb) / 100,
				ultrasound_peak_dBuV: parsed.slice(16, 18).reduce(msbLsb),
				peak_to_peak_mV: parsed.slice(18, 20).reduce(msbLsb) / 100,
				ultrasound_peak_to_peak_dBuV: parsed.slice(20, 22).reduce(msbLsb),
				crest_factor_dB: parsed[22],
				freq_1: parsed.slice(23, 25).reduce(msbLsb),
				freq_2: parsed.slice(25, 27).reduce(msbLsb),
				freq_3: parsed.slice(27, 29).reduce(msbLsb)
			};
		}
