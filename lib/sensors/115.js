	...
	const parse = (payload, parsed, mac) => {
		...
		if (vibration_mode === 0) { // basic mode
			...
			return {
				...
				crest_factor_dB: parsed.slice(22, 24).reduce(msbLsb) / 100,
				temperature: parsed.slice(24, 26).reduce(msbLsb) / 100,
				freq_1: parsed.slice(26, 28).reduce(msbLsb),
				freq_2: parsed.slice(28, 30).reduce(msbLsb),
				freq_3: parsed.slice(30, 32).reduce(msbLsb)
			};
		}
