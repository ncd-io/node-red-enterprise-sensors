	...
	const parse = (payload, parsed, mac) => {
		...
		if (vibration_mode === 0) { // basic mode
			return {
				...
				rms_mV: parsed.slice(10, 12).reduce(msbLsb) / 100,
			...
			return {
				...
				rms_mV: parsed.slice(10, 12).reduce(msbLsb) / 100,
				rms_ultrasound_dBuV: parsed.slice(12, 14).reduce(msbLsb),
				peak_mV: parsed.slice(14, 16).reduce(msbLsb),
				ultrasound_peak_dBuV: parsed.slice(16, 18).reduce(msbLsb),
				peak_to_peak_mV: parsed.slice(18, 20).reduce(msbLsb),
				ultrasound_peak_to_peak_dBuV: parsed.slice(20, 22).reduce(msbLsb),
				crest_factor_dB: parsed.slice(22, 24).reduce(msbLsb),
				freq_1: parsed.slice(24, 26).reduce(msbLsb),
				freq_2: parsed.slice(26, 28).reduce(msbLsb),
				freq_3: parsed.slice(28, 30).reduce(msbLsb)
			};
		}
