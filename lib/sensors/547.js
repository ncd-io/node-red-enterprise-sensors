...
module.exports = (globalDevices) => {
	...
	const get_config_map = (firmware) => {
		...
		return {
			...
			"h2_sensor_boot_time": {
				...
				"html_id": "h2_sensor_boot_time_547"
			}
		};
	};
	...
	const parse = (payload, parsed, mac) => {
		let bme_status_byte = payload[7] & 0x07;
		let hydro_status_byte = payload[7] & (1 << 3);

		let bme_status = 'sucess';
		switch(bme_status_byte){
			case 1:
				bme_status = {
					error: 'Connection failed',
					value: bme_status_byte
				}
				break;
			case 2:
				bme_status = {
			...
			case 5:
				bme_status = {
					error: 'Sensor generic error',
					value: bme_status_byte
				}
				break;
		}
		
		let hydro_status = hydro_status_byte?'Failure':'sucess'; 

		return{
			environment_status: bme_status,
			hydro_status: hydro_status,
			h2_concentration_ppm: payload.slice(8, 12).reduce(msbLsb),
			temperature: signInt(payload.slice(12, 14).reduce(msbLsb), 32) / 100,
			pressure: payload.slice(14, 18).reduce(msbLsb) / 100,
			humidity: payload.slice(18, 22).reduce(msbLsb) / 1000,
			resistance: payload.slice(22, 26).reduce(msbLsb),
			iaq: payload.slice(26, 28).reduce(msbLsb),
			co2_eqv: payload.slice(28, 30).reduce(msbLsb),
			breath_voc: payload.slice(30, 34).reduce(msbLsb) / 100,
			static_iaq: payload.slice(34, 38).reduce(msbLsb) / 100,
			iaq_accuracy: payload[38]
		};
	};
	...
	return {
		type: 547,
		name: 'Wireless Hydrogen Enviorment Sensor',
		parse,
