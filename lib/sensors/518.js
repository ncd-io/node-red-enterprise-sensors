...
module.exports = (globalDevices) => {
		...
		return {
			...
			"sampling_rate": {
				...
				"descriptions": {
					...
					"main_caption": "Interval in seconds between sensor measurements"
				},
				"default_value": 1800,
				"validator": {
					"type": "uint16be",
			...
			"sampling_duration": {
				...
				"descriptions": {
					...
					"main_caption": "Time in seconds after which sampling is stopped"
				},
				"default_value": 0,
				"validator": {
					"type": "uint16be",
	...
	const parse = (payload, parsed, mac) => {
		if(parsed.firmware > 5){
			let status = parsed[7] >> 1; // (1-bit left shifted)
			if(status){
				status = 'read_failure';
			} else {
				status = 'read_success';
			...
			return {
				status: status,
				pressure: signInt(payload.slice(8, 12).reduce(msbLsb), 32) / 1000,
				temperature: signInt(payload.slice(12, 14).reduce(msbLsb), 16) / 100,
				velocity: signInt(payload.slice(14, 16).reduce(msbLsb), 16) / 1000, // meter/sec
				inches_of_water_H2O: signInt(payload.slice(16, 19).reduce(msbLsb), 32) / 1000
			};
		}else{
			return {
				pressure: signInt(payload.slice(8, 12).reduce(msbLsb), 32) / 1000,
				temperature: signInt(payload.slice(12, 14).reduce(msbLsb), 16) / 100,
				velocity: signInt(payload.slice(14, 16).reduce(msbLsb), 16) / 1000
			};
		}
	};
