...
module.exports = (globalDevices) => {
		...
		return {
			...
			"sps_skipped_samples": {
				...
				"descriptions": {
					...
					"main_caption": "Number of initial unstable particulate samples the sensor discards before transmitting, so each reading reflects a settled measurement."
				},
				"default_value": 7,
				"validator": {
					"type": "uint8",
				...
				"html_id": "sps_skip_samples_32"
			},
			"sps_cleaning_rate": {
				"read_index": 26,
				"write_index": 15,
				"descriptions": {
					"title": "Set Cleaning Rate (Particulate)",
					"main_caption": "Sets the number of transmitted reports before the sensor runs a high-speed fan self-cleaning cycle. Lower values will increase average power consumption."
				},
				"default_value": 10,
				"validator": {
					"type": "uint16be",
	...
	const parse = (d, payload) => {
		...
		let sht_status = status_byte & 1;
		let sps_status = status_byte & 2;
		...
		if(sps_status || sht_status){
			res.status = {};
			if (sps_status) {
				res.status.sps_sensor = "error";
			}
			if (sht_status) {
				res.status.sht_sensor = "error";
			}
		}
		return res;
	};
