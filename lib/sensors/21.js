...
module.exports = (globalDevices) => {
	...
	const parse = (payload, parsed, mac) => {
		...
		if(parsed.firmware > 14){
			...
			let status = reserved >> 1; // (1-bit left shifted)
			if(status){
				status = 'read_failure';
			} else {
				status = 'read_success';
			...
			return {
				...
				raw_adc: payload.slice(14, 18).reduce(msbLsb), // raw_adc 4 bytes
				inches_of_water_H2O: signInt(payload.slice(18, 22).reduce(msbLsb), 32) / 1000
			};
		}
