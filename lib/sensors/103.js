...
module.exports = (globalDevices) => {
	...
	const parse = (payload, parsed, mac) => {
		...
		let acc_error = (reserved & 0x02) !== 0;
		let gyro_error = (reserved & 0x04) !== 0;
		let status = 'Both Enabled';

		if (acc_error && gyro_error) {
			parsed.data = { error: 'Error found, Gyro and Acc error' };
			return parsed;
		} else if (gyro_error) {
			status = 'Gyro unattached or disabled';
		} else if (acc_error) {
			status = 'Acc unattached or disabled';
		}
