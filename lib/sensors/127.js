...
module.exports = (globalDevices, emitter) => {
	...
	const init_globalDevices_stream = (deviceAddr, payload, expected_packets, parsed, msg_type) => {
		// Determine the source of the raw message (accelerometer or ultrasound sensor) 
		// by checking the value of bit 5 in the reserved byte.
		var source_type = (payload[7] & 0x20) >> 5;

		if (!source_type) { // Accelerometer
			globalDevices[deviceAddr] = {
				source_type: source_type,
				data: {},
				mo: payload[8],
				odr: payload.slice(9, 11).reduce(msbLsb),
				fsr: payload[11] >> 5,
				hour: payload[12],
				minute: payload[13],
			...
			// ODR MSB of sampling freq + lower two bytes of sampling freq
			let odr = [payload[11], payload[9], payload[10]];

			globalDevices[deviceAddr] = {
				source_type: source_type,
				data: {},
				mo: payload[8],
				odr: odr.slice(0, 3).reduce(msbLsb),
				hour: payload[12],
				minute: payload[13],
				temperature: msbLsb(payload[14], payload[15]) / 100,
				expected_packets: expected_packets
		...
		globalDevices[deviceAddr].packet_stream_timeout = setTimeout(() => {
			...
			emitter.emit('sensor_data-127', parsed);
	...
	const concat_fft_data = (deviceAddr, mode, msg_type) => {
		...
		// Accelerometer
		if (!globalDevices[deviceAddr].source_type) {
			...
			var fft_concat = [];
			var increment = 2;
			var fsr_text = "120dbuV";
			var fst_mult = 0.2889966;

			for (var i = 0; i < raw_data.length; i += increment) {
				label++;
				fft_concat.push(parseFloat((signInt(((raw_data[i] << 8) + (raw_data[i + 1])), 16) * fst_mult).toFixed(3)));
			}
		}

		var fft_concat_obj = {
			mode: mode,
			source_type: globalDevices[deviceAddr].source_type ? 'ultrasound' : 'accelerometer',
			msg_type: msg_type,
			time_id: [
		...
		return {
			...
			"raw_packet_length": {
				...
				"descriptions": {
					...
					"note": "<p class=\"caption\"><i>Note: For the 2.4GHz version you need to operate with a 55 Byte payload.</i></p>"
				},
				"default_value": 3,
				"validator": {
					"type": "uint8",
			...
			"auto_raw_interval": {
				...
				"descriptions": {
					...
					"note": "<p class=\"caption\"><i>Note: Auto Raw Transmission is disabled by default.</i></p>"
				},
				"default_value": 0,
				"validator": {
					"type": "uint8",
			...
			"auto_raw_destination_address": {
				...
				"descriptions": {
					...
					"note": "<p class=\"caption\">Default value: 0000FFFF for Broadcast Mode</p>"
				},
				"default_value": "0000FFFF",
				"validator": {
					"type": "mac",
			...
			"ultrasound_max_raw_samples": {
				"read_index": 52,
				"write_index": 41,
				"descriptions": {
					"title": "Set Ultrasound Max Raw Sample",
					"main_caption": "<p>Set the maximum number of samples...</p>"
				},
				"default_value": 0,
				"validator": {
				...
				"html_id": "ultrasound_max_raw_sample_127"
			},
			"probe_uptime": {
				"read_index": 53,
				"descriptions": {
	...
	const parse = (payload, parsed, mac) => {
		...
		if (payload[8] === 1) {
			...
			return {
				...
				ultrasonic_rms_dBuV: payload.slice(56, 58).reduce(msbLsb),
				ultrasonic_peak_to_peak_dBuV: payload.slice(58, 60).reduce(msbLsb),
				crest_factor: payload.slice(60, 62).reduce(msbLsb) / 100
			};
		}
	};
	...
	return {
		type: 127,
		name: 'Wireless Vibration, Ultrasound and Temperature Sensor',
		parse,
