...
module.exports = (globalDevices, emitter) => {
	...
	const init_globalDevices_stream = (deviceAddr, payload, expected_packets, parsed, msg_type) => {
		...
		var raw_data_source_type = (payload[7] & 0x20) >> 5;

		if (!raw_data_source_type){ // Accelerometer
			...
			globalDevices[deviceAddr] = {
				raw_data_source_type: raw_data_source_type,
				data: {},
				odr: payload.slice(9, 13).reduce(msbLsb),
				mo: payload[8],
				fsr: payload[13],
				hour: payload[14],
				minute: payload[15],
	...
	const concat_fft_data = (deviceAddr, mode, msg_type) => {
		...
		// Accelerometer
		if(!globalDevices[deviceAddr].raw_data_source_type){
		...
		var fft_concat_obj = {
			mode: mode,
			raw_data_source_type: globalDevices[deviceAddr].raw_data_source_type ? 'ultrasound' : 'accelerometer',
			msg_type: msg_type,
			time_id: [
			...
			...(!globalDevices[deviceAddr].raw_data_source_type && { temperature: globalDevices[deviceAddr].temperature }),
			total_samples: label,
