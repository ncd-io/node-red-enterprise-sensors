...
module.exports = class WirelessSensor{
		...
		if (fs.existsSync(sensors_path)) {
			fs.readdirSync(sensors_path).forEach(file => {
				if (file.endsWith('.js')) {
					...
					const sensor = sensorFactory(this.globalDevices);
					
					// Register the initialized library by type ID
					this.sensor_libs[sensor.type] = sensor;
					
					console.log(`Loaded sensor type ${sensor.type}: ${sensor.name}`);
				}
			});
		}
	};
	...
	sync_ack(payload, frame){
		...
		if(rep_buffer[2] === 0xff ){
			...
			this._emitter.emit('sync_acknowledgment-'+frame.mac, {
				error: 'Sync Configurations Successful',
				response: rep_buffer
			});
		}else{
			report = {
			...
			this._emitter.emit('sync_acknowledgment_error-'+frame.mac, {
				error: 'Sync Configurations Failed',
				response: rep_buffer
			});
		}
	...
	build_sync_command(configs) {
		...
		return [...buf]; // Convert to integer array for the radio
	}
	send_sync_configs(sensor_mac, configs){
		console.log('send_sync_configs command');
		const packet = this.build_sync_command(configs);
		console.log('Built sync packet: ');
		console.log(packet);
		return this.config_send_sync(sensor_mac, packet);
	};
			...
			'utils': {
				...
				'concat_fft_data': (deviceAddr, mode, msg_type) => {
					...
					for(var i = 0; i < raw_data.length; i+=en_axis_data.increment){
						...
						if('x_offset' in en_axis_data){
							fft_concat.x.push(parseFloat((signInt(((raw_data[i+en_axis_data.x_offset]<<8)+(raw_data[i+en_axis_data.x_offset+1])), 16)*fsr_mult).toFixed(4)));
						}
						if('y_offset' in en_axis_data){
							fft_concat.y.push(parseFloat((signInt(((raw_data[i+en_axis_data.y_offset]<<8)+(raw_data[i+en_axis_data.y_offset+1])), 16)*fsr_mult).toFixed(4)));
						}
						if('z_offset' in en_axis_data){
							fft_concat.z.push(parseFloat((signInt(((raw_data[i+en_axis_data.z_offset]<<8)+(raw_data[i+en_axis_data.z_offset+1])), 16)*fsr_mult).toFixed(4)));
						}
					}
					var fft_concat_obj = {
						mode: mode,
			...
			parse: (d, parsed, mac) => {
				...
				if(firmware > 8){
					if(d[7] & 2){
						...
						return {error: 'Error found, Load cell comm error'};
					}
				}else{
					if(d[7] == 254){
						console.log('Error found');
