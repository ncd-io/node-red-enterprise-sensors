...
function sensor_types(parent){
	var types = {
		...
		'58': {
			...
			parse: (d) => {
				return {
					filtered_range: d.slice(0, 2),
					long_range_algorithm: d.slice(2, 4),
					second_chance:  d.slice(4, 6),
					copare_reading:  d.slice(6, 8),
					short_range_algorithm: d.slice(8, 10),
					trusted_reading:  d.slice(10, 12),
					final_filter:  d.slice(12, 14),
					raw_final:  d.slice(14, 16),
					final_index:  d.slice(16, 18),
				};
			}
		},
		'60': {
		...
		'531': {
			...
			parse: (d, p, mac) => {
				...
				if(d[8] === 1){
					let deviceAddr = mac;
					let expected_packets = d[10];
					let current_packet = d[11];
					let sdata_start = 12;
					...
					if(globalDevices.hasOwnProperty(deviceAddr) || expected_packets == 1){
						if(expected_packets != 1){
						 //...
						    ...
						    globalDevices[deviceAddr].data[current_packet] = d.slice(sdata_start);
						}
						else{
							var mode = d[8];
							var sample_rate = d[9];
						...
						if(Object.keys(globalDevices[deviceAddr].data).length == expected_packets){
							...
							for(let i = 0; i< raw_data.length; i++){
								label++;
								adc[label] = {};
								adc[label] = signInt((raw_data[i] << 8)+(raw_data[i+1]<<8), 16) * 89;
							}
							
							let concat_obj = {
								mode: globalDevices[deviceAddr].mode,
								sample_rate: globalDevices[deviceAddr].sample_rate,
								data: adc
							}
							sensor_data = concat_obj;
							delete globalDevices[deviceAddr];
							return sensor_data;
						}
						else{
				...
				if(d[8] === 0){
					...
					if(firmware > 1){
						return {
							...
							rms_dba: d[10],
							c1_dba: d[11],
							c1_freq: d.slice(12, 14).reduce(msbLsb),
							c2_dba: d[14],
							c2_freq: d.slice(15, 17).reduce(msbLsb),
							c3_dba: d[17],
							c3_freq: d.slice(18, 20).reduce(msbLsb),
						};
					}else{
						return {
							noise_db: d[0],
