// ERROR CODES - last four digits relate to sensor type prior digits refer to ID
// 1000080 - multi-packet stream corrupted/missing data
// 1000081 - multi-packet stream corrupted/missing data on probe x
// 1100080 - multi-packet stream timed out
// 1100081 - multi-packet stream timed out on probe x
// 1200080 - No Time Series Data Received
// 1200081 - No Time Series Data Received from multi-probe
// 2000080 - Sensor probe unattached
// 2100080 - Sensor Probe 1 unattached on t81
// 2200080 - Sensor Probe 2 unattached on t81
...
module.exports = class WirelessSensor{
	...
	parse(frame){
		...
		if(typeof this[type] == 'function'){
			...
			// #OTF
			var otf_devices = [23,26,39,44,45,48,52,78,79,80,81,82,84,88,89,91,101,102,107,108,519,520,521,531,537];
	...
	request_time_series(data){
		// msg.payload = {
		// 	address: "00:13:a2:00:42:2c:d2:aa",
		// 	sensor_type: 80, //OPTIONAL only required for t81 sensor
		...
		// Make sure object exists so we can assign stream timeout.
		if(!Object.hasOwn(globalDevices, data.address)){
			globalDevices[data.address] = {};
		}
		// Prevents duplicate stream timeouts if multiple commands sent
		if(Object.hasOwn(globalDevices[data.address], 'stream_timeout')){
			clearTimeout(globalDevices[data.address].stream_timeout);
			delete globalDevices[data.address].stream_timeout;
		}

		if(Object.hasOwn(data, 'sensor_type') && data.sensor_type == 81){
			if(Object.hasOwn(data, 'probe')){
				if(!Object.hasOwn(globalDevices[data.address], data.probe)){
					globalDevices[data.address] = {1: {}, 2: {}};
				}
				switch(data.probe){
					case 1:
						command = [0xf4, 0x4f, 0x00, 0x00, 0x00, 0x13, 0x01];
						break;
					case 2:
						command = [0xf4, 0x4f, 0x00, 0x00, 0x00, 0x13, 0x02];
						break;
					default:
						console.log('No valid probe number Provided for type 81');
						return;
				}
				// Have to handle timeout seperately for t81 as the data objects are handled seperately per probe.
				globalDevices[data.address][data.probe].stream_timeout = setTimeout(() => {
					let error_msg = {
						error: {
							code: 1200081,
							text: 'No data from time series request received within timeout on probe ' + data.probe,
							probe: data.probe
						},
						addr: data.address,
						timeout: 10000
					};
					if(Object.hasOwn(data, 'sensor_type')){
						error_msg.sensor_type = data.sensor_type;
						this._emitter.emit('ncd_error-'+data.sensor_type, error_msg);
					};
					this._emitter.emit('ncd_error', error_msg);
					this._emitter.emit('ncd_error-'+data.address, error_msg);

				}, 10000);
			}
		}else{
			command = [0xf4, 0x4f, 0x00, 0x00, 0x00, 0x13];

			// This timeout is useful for the reception of the first packet, but comms cut afterward
			globalDevices[data.address].stream_timeout = setTimeout(() => {
				let error_msg = {
					error: {
						code: 1200080,
						text: 'No data from time series request received within timeout',
						probe: 1
					},
					addr: data.address,
					timeout: 10000
		...
		// 	globalDevices[data.address] = {};
		// }
		this.control_send(data.address, command).then(res => console.log(res)).catch(error => console.log(error));
	}
	route_discover(sensor_mac, data, opts){
		// var data = [85];
		var that = this;
		return new Promise((fulfill, reject) => {
			that.queue.add(() => {
...
function sensor_types(parent){
	var types = {
		...
		'63': {
			...
			parse: (d) => {
				return {
					...
					ORP_Temp: signInt(d.slice(2, 4).reduce(msbLsb),16) / 100,
					pH: signInt(d.slice(4, 6).reduce(msbLsb), 16) / 100,
					pH_Temp: signInt(d.slice(6, 8).reduce(msbLsb),16) / 100
				};
			}
		},
		'64': {
				...
				if(payload[8] === 1){
					...
					if(Object.hasOwn(globalDevices[deviceAddr], 'data') && Object.keys(globalDevices[deviceAddr].data).length == expected_packets && !Object.hasOwn(globalDevices[deviceAddr], 'err_msg_counter_val')){
						...
						for(var i = 0; i < raw_data.length; i+=en_axis_data.increment){
							...
							if('x_offset' in en_axis_data){
								fft_concat[label].x = parseFloat((signInt(((raw_data[i+en_axis_data.x_offset]<<8)+(raw_data[i+en_axis_data.x_offset+1])), 16)*fsr_mult).toFixed(3));
							}
							if('y_offset' in en_axis_data){
								fft_concat[label].y = parseFloat((signInt(((raw_data[i+en_axis_data.y_offset]<<8)+(raw_data[i+en_axis_data.y_offset+1])), 16)*fsr_mult).toFixed(3));
							}
							if('z_offset' in en_axis_data){
								fft_concat[label].z = parseFloat((signInt(((raw_data[i+en_axis_data.z_offset]<<8)+(raw_data[i+en_axis_data.z_offset+1])), 16)*fsr_mult).toFixed(3));
							}
						}
						var fft_concat_obj = {
							time_id: globalDevices[deviceAddr].hour +':'+ globalDevices[deviceAddr].minute,
		...
		'81': {
			...
			parse: (payload, parsed, mac) => {
				...
				if(payload[7] & 2){
					parsed.error = {
						code: 2100080,
						text: 'Sensor Probe 1 may be unattached',
						probe: 1
					};
					parsed.addr = mac;
				...
				if(payload[7] & 4){
					parsed.error = {
						code: 2200080,
						text: 'Sensor Probe 2 may be unattached',
						probe: 2
					};
					parsed.addr = mac;
				...
				if(payload[8] === 1){
					...
					if(!Object.hasOwn(globalDevices[deviceAddr], 1)){
						globalDevices[deviceAddr][1] = {};
					}
					if(!Object.hasOwn(globalDevices[deviceAddr], 2)){
						globalDevices[deviceAddr][2] = {};
					}
					// Check if new stream is from new message
					if(Object.hasOwn(globalDevices[deviceAddr][probe], 'err_msg_counter_val') && globalDevices[deviceAddr][probe].err_msg_counter_val != parsed.counter){
						delete globalDevices[deviceAddr][probe].err_msg_counter_val;
					}
					...
					// This section check if data does and if current_packet is not 1. This indicates the data started mid stream
					// !Object.hasOwn(globalDevices[deviceAddr], 'data') && current_packet != 1
					if(Object.hasOwn(globalDevices[deviceAddr][probe], 'data') && current_packet in globalDevices[deviceAddr][probe].data || Object.hasOwn(globalDevices[deviceAddr][probe], 'data') && !(((current_packet&127)-1) in globalDevices[deviceAddr][probe].data) || !Object.hasOwn(globalDevices[deviceAddr][probe], 'data') && current_packet != 1){
						// if(!Object.hasOwn(globalDevices[deviceAddr], 'err_msg_counter_val')){
						//	 globalDevices[deviceAddr].err_msg_counter_val = parsed.counter;
						// }
						// If Data exists that means we're mid packet and an error occured.
						if(Object.hasOwn(globalDevices[deviceAddr][probe], 'data')){
							// console.log('-----');
							// console.log(deviceAddr+': bad packet breakdown deleting stream');
							// console.log(current_packet);
							// console.log(expected_packets);
							...
							// console.log(current_packet == 1);
							// console.log(!((current_packet-1) in globalDevices[deviceAddr].data));
							delete globalDevices[deviceAddr][probe].data;
						}
						// If we have not already reported an error
						if(!Object.hasOwn(globalDevices[deviceAddr][probe], 'err_msg_counter_val')){
							globalDevices[deviceAddr][probe].err_msg_counter_val = parsed.counter;
							if(Object.hasOwn(globalDevices[deviceAddr][probe], 'stream_timeout')){
								clearTimeout(globalDevices[deviceAddr][probe].stream_timeout);
								delete globalDevices[deviceAddr][probe].stream_timeout;
							}
							parsed.error = {
								code: 1000081,
								text: 'Faulty multi-packet stream detected on probe ' + probe,
								probe: probe
							}
							parsed.addr = mac;
							parent._emitter.emit('ncd_error', parsed);
					...
					if(current_packet == 1){
						...
						// This timeout is useful for the reception of the first packet, but comms cut afterward
						if(expected_packets != 1){
							globalDevices[deviceAddr][probe].stream_timeout = setTimeout(() => {
								if(Object.hasOwn(globalDevices[deviceAddr][probe], 'data')){
									delete globalDevices[deviceAddr][probe].data;
								}
								parsed.error = {
									code: 1100081,
									text: 'Multi-Packet Stream started, but timed out on probe ' + probe,
									probe: probe
								}
								parsed.addr = mac;
								parent._emitter.emit('ncd_error', parsed);
					...
					}else if(Object.hasOwn(globalDevices[deviceAddr][probe], 'data')){
						// Not first packet and no error detected, append to data array
						globalDevices[deviceAddr][probe].data[current_packet] = payload.slice(sdata_start);
					}

					if(Object.hasOwn(globalDevices[deviceAddr][probe], 'data') && Object.keys(globalDevices[deviceAddr][probe].data).length == expected_packets && !Object.hasOwn(globalDevices[deviceAddr][probe], 'err_msg_counter_val')){
						var raw_data = new Array();
						...
						var fft_concat_obj = {
							probe: probe,
							time_id: globalDevices[deviceAddr][probe].hour +':'+ globalDevices[deviceAddr][probe].minute,
							mac_address: deviceAddr,
							en_axis: globalDevices[deviceAddr][probe].en_axis,
							fsr: fsr_text,
							odr: globalDevices[deviceAddr][probe].odr,
							device_temp: globalDevices[deviceAddr][probe].device_temp,
							data: fft_concat
						};
						sensor_data = fft_concat_obj;
						// Clear stream timeout to prevent timeout error message from triggering
		...
		'108': {
			name: 'Machine Uptime Monitor',
			parse: (d) => {
				return {
					ac_current: (signInt(d.slice(0, 2).reduce(msbLsb)))/100.0,
					input_1_counter: signInt(d.slice(2, 6).reduce(msbLsb)),
					input_1_timer: signInt(d.slice(6, 10).reduce(msbLsb)),
					input_2_counter: signInt(d.slice(10, 14).reduce(msbLsb)),
					input_2_timer: (signInt(d.slice(14, 18).reduce(msbLsb))),
					input_3_counter: (signInt(d.slice(18, 22).reduce(msbLsb))),
					input_3_timer: (signInt(d.slice(22, 26).reduce(msbLsb))),
					accelerometer_counter: (signInt(d.slice(26, 30).reduce(msbLsb))),
					accelerometer_timer: (signInt(d.slice(30, 34).reduce(msbLsb))),
					magnetometer_counter: (signInt(d.slice(34, 38).reduce(msbLsb))),
					magnetometer_timer: (signInt(d.slice(38, 42).reduce(msbLsb)))
				};
			}
		},
		'200': {
