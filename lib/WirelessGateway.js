...
module.exports = class WirelessSensor{
	...
	parse(frame){
		...
		if(typeof this[type] == 'function'){
			...
			// #OTF
			var otf_devices = [4,12,21,23,26,32,33,39,44,45,48,52,53,56,58,74,76,78,79,80,81,82,84,88,89,90,91,97,98,101,102,105,106,107,108,110,111,112,114,118,180,181,202,519,520,521,531,535,537,538,539,540,1010,1011];
	...
	var types = {
		...
		'110': {
			...
			'parse_fly': (frame) => {
				...
				frame_data.auto_raw_interval = frame[32] * frame_data.sampling_interval_number;
				...
				return {
					...
					'auto_raw_interval': frame_data.auto_raw_interval+'min',
					'smart_mode_skip_count': frame[33],
					'smart_mode_acc_threshold':frame_data.smart_mode_threshold+'mg',
					'hardware_id': frame.slice(35, 38),
					'reserved': frame.slice(38, 41),
					'tx_lifetime_counter': frame.slice(41, 44).reduce(msbLsb),
					'machine_values': {
						'firmware': frame[2],
						'destination_address': toMac(frame.slice(12, 16), false),
						'mode': frame[16],
						...
						'hardware_id': frame.slice(35, 38),
						'reserved': frame.slice(38, 40),
						'tx_lifetime_counter': frame.slice(40, 42)
					}
				}
			}
		},
		'111': {
			...
			'parse_fly': (frame) => {
				...
				frame_data.auto_raw_interval = frame[36] * frame_data.sampling_interval_number;
				...
				return {
					...
					'reserved': frame.slice(43, 46),
					'tx_lifetime_counter': frame.slice(46, 49).reduce(msbLsb),
					'machine_values': {
						'firmware': frame[2],
						'destination_address': toMac(frame.slice(12, 16), false),
						'mode': frame[16],
			...
			'parse_fly': (frame) => {
				...
				return {
					...
					'machine_values': {
						...
						'hardware_id': frame.slice(35, 38),
						'reserved': frame.slice(38, 41),
						'tx_lifetime_counter': frame.slice(41, 44)
					}
				}
			}
		},
