...
module.exports = class WirelessSensor{
	...
	parse(frame){
		...
		if(typeof this[type] == 'function'){
			...
			// #OTF
			var otf_devices = [4,12,21,23,26,32,33,39,44,45,48,52,53,56,58,74,76,78,79,80,81,82,84,88,89,90,91,97,98,101,102,103,105,106,107,108,109,110,111,112,114,117,118,120,121,122,123,180,181,202,217,519,520,521,531,535,537,538,539,540,1010,1011];
	...
	var types = {
			...
			'parse_fly': (frame) => {
				...
				if(firmware > 13){ // firmware 14 and above
					...
					return {
						...
						'calibration_one': frame.slice(36, 40).reduce(msbLsb),
						'calibration_two':frame.slice(40, 44).reduce(msbLsb),
						'calibration_three':frame.slice(44, 48).reduce(msbLsb),
						'machine_values': {
							'firmware': frame[2],
							'fsr': frame[16],
							'boot_time': frame[17],
							...
							'calibration_one': frame.slice(36, 40),
							'calibration_two':frame.slice(40, 44),
							'calibration_three':frame.slice(44, 48)
						}
					}
				}
			}
		...
		'103': {
				...
				if(payload[9] === 0){ // mode raw
					...
					if(globalDevices.hasOwnProperty(deviceAddr) || expected_packets == 1){
						...
						if(Object.keys(globalDevices[deviceAddr].data).length == expected_packets){
							...
							var en_axis_data = {};
							switch (globalDevices[deviceAddr].en_axis){ // it will be 7 all time
								case 7:
									en_axis_data.x_offset = 0;
									en_axis_data.y_offset = 2;
									en_axis_data.z_offset = 4;
			...
			'parse_fly': (frame) => {
				...
				switch(frame[12]){
					case 0:
						frame_data.odr = '125Hz';
						break;
					case 1:
						frame_data.odr = '250Hz';
						break;
					case 2:
						frame_data.odr = '500Hz';
						break;
					case 3:
						frame_data.odr = '1000Hz';
						break;
					default:
						frame_data.odr = 0;
				}
				...
				switch(frame[20]){
					case 1:
						...
						break;
					case 2:
						frame_data.en_sensors = "adxl_only";
						break;
					case 3:
						frame_data.en_sensors = "all_enabled";
						break;
				}
				frame_data.hpf = frame[14]?'Enabled':'Disabled';
				return {
					'firmware': frame[2],
					'odr': frame_data.odr,
					'sampling_duration': frame[13] + 'ms',
					'hpf_cutoff': frame_data.hpf,
					'acc_fsr': frame_data.fsr_acc,
					'axis_enabled': frame_data.en_axis,
					'gyro_fsr': frame_data.fsr_gyro,
					'sampling_interval': frame[18] + 'min',
			...
			'parse_fly': (frame) => {
				if(frame[2] > 9){ // firmware 10 and above
					...
					return {
						...
						'debouncing_timeout': frame.slice(17, 19).reduce(msbLsb) + "sec",
		...
		'123': {
			...
			'parse_fly': (frame) => {
				...
				return {
					'firmware': frame[2],
					'debouncing_timeout': frame.slice(16, 18).reduce(msbLsb) + "sec",
					...
					'counter_threshold': frame.slice(21, 25).reduce(msbLsb) + "sec",
