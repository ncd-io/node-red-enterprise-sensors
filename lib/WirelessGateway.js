...
module.exports = class WirelessSensor{
	...
	sensor_data(payload, frame){
		...
		if(payload.length == 179){
			...
			if(typeof this.sensor_types[parsed.sensor_type] == 'undefined' && typeof this.sensor_libs[parsed.sensor_type] == 'undefined'){
				...
				// #OTF
			}else if(parsed.sensor_type == 20 || parsed.sensor_type == 21 || parsed.sensor_type == 24 || parsed.sensor_type == 26 || parsed.sensor_type == 33 || parsed.sensor_type == 39 || parsed.sensor_type == 29 || parsed.sensor_type == 51 || parsed.sensor_type == 54 || parsed.sensor_type == 55 || parsed.sensor_type == 60 || parsed.sensor_type == 80 ||  parsed.sensor_type == 81 || parsed.sensor_type == 82 || parsed.sensor_type == 84 || parsed.sensor_type == 86 || parsed.sensor_type == 87 || parsed.sensor_type == 97 || parsed.sensor_type == 98 || parsed.sensor_type == 99 || parsed.sensor_type == 103 || parsed.sensor_type == 105 || parsed.sensor_type == 110 || parsed.sensor_type == 111 || parsed.sensor_type == 112 || parsed.sensor_type == 114 || parsed.sensor_type == 117 || parsed.sensor_type == 118 || parsed.sensor_type == 127 || parsed.sensor_type == 128 || parsed.sensor_type == 129 || parsed.sensor_type == 130 || parsed.sensor_type == 131 || parsed.sensor_type == 180 || parsed.sensor_type == 181 || parsed.sensor_type == 202 || parsed.sensor_type == 217 || parsed.sensor_type == 515 || parsed.sensor_type == 518 || parsed.sensor_type == 519 || parsed.sensor_type == 520 || parsed.sensor_type == 531 || parsed.sensor_type == 537 || parsed.sensor_type == 538 || parsed.sensor_type == 543 || parsed.sensor_type == 545 || parsed.sensor_type == 547 || parsed.sensor_type == 554){
	...
	config_set_always_on_26(sensor_mac, value){
		console.log('config_set_always_on_26');
	...
	link_test(source_mac,destination_mac,testOpts){
		var that = this;
		//Test parameters. Both go on the wire as big-endian uint16, so the full
		//1-65535 range is supported; omitting them keeps the historical 200x200
		//behaviour, so existing callers are unaffected.
		//
		//These used to be hardcoded, and the caller's third argument was shadowed
		//by the local frame-options object below before it could ever be read.
		var iterations_count = clampUint16(testOpts && testOpts.packets, 200);
		var payload_bytes = clampUint16(testOpts && testOpts.packetSize, 200);
		return new Promise((fulfill, reject) => {
			that.queue.add(() => {
				return new Promise((f, r) => {
					var failed = false;
					...
					function send(){
						...
						var payload_size = [(payload_bytes >> 8) & 255, payload_bytes & 255];
						var iterations = [(iterations_count >> 8) & 255, iterations_count & 255];
						...
						//Renamed from `opts` so it no longer shadows the caller's
						//test parameters.
						var frameOpts = {"method":3};
						that.send.explicit_addressing_command(mac2bytes(source_mac), 230, 230, cluster, profile, data, frameOpts).then(function(frame){
							if(frame.delivery_status == 'Success'){
								pass(frame);
							}else{
								tO = setTimeout(() => {
	...
	var types = {
		...
		'97': {
			...
			'parse_fly': (frame) => {
				...
				switch(frame[13]){
					case 0:
						frame_data.raw_length = 55;
					break;
					case 1:
						frame_data.raw_length = 100;
					break;
					case 2:
						frame_data.raw_length = 150;
					break;
					case 3:
						frame_data.raw_length = 180;
					break;
				}
				return {
					'firmware': frame[2],
					'mode': frame_data.mode,
					'raw_length': frame_data.raw_length + " Bytes",
...
//Coerce a link-test parameter into the uint16 the frame carries, falling back to
//`dflt` for anything missing or not a number. Callers reach this from user input,
//so a bad value must clamp rather than reach the radio.
function clampUint16(v, dflt){
	var n = parseInt(v, 10);
