	...
	sensor_data(payload, frame){
		...
		if(payload.length == 179){
			...
			if(typeof this.sensor_types[parsed.sensor_type] == 'undefined'){
				...
				// Added new if to check if we need to pass in this for emitter in stream timeout.
				if(parsed.sensor_type == 114){
...
function sensor_types(parent){
	var types = {
		...
		'114': {
			...
			parse: (payload, parsed, mac, parent) => {
				if(payload[7] & 2){
					console.log('Error found');
					parsed.data = {error: 'Error found, Sensor Probe may be unattached'};
					return parsed;
				...
				if(payload[8] === 1){
					...
					if(globalDevices.hasOwnProperty(deviceAddr) || expected_packets == 1){
						...
						globalDevices[deviceAddr].last_packet_counter = current_packet;
						globalDevices[deviceAddr].data[current_packet] = payload.slice(sdata_start);
						globalDevices[deviceAddr].expected_packets = expected_packets;
						// 60 second timeout for packet stream completion
						globalDevices[deviceAddr].packet_stream_timeout = setTimeout(() => {
							...
							// console.log('Packet stream timeout - clearing ' + deviceAddr);
							// console.log(parent);
							// console.log(parent._emitter);
							// parsed.sensor_data = {error: 'Time Series Data Stream Timeout - incomplete data received'};	
							parent._emitter.emit('sensor_data', parsed);
							parent._emitter.emit('sensor_data-114', parsed);
							parent._emitter.emit('sensor_data'+'-'+mac, parsed);
						}, 120000);
					}
					if(current_packet == expected_packets){
						var raw_data = new Array();
			...
			parse: (d, parsed, mac) => {
				...
				if(firmware > 4){
					...
					return{
						...
						unit: unit,
						raw_weight: signInt(d.slice(13, 17).reduce(msbLsb), 32)
					}
				}
				else {
					return {
