			...
			parse: (payload, parsed, mac) => {
				...
				if(payload[8] === 1){
					...
					return {
						...
						z_ppv_velocity: payload.slice(30, 31).reduce(msbLsb) / 100,
						z_peak_one_Hz: payload.slice(31, 33).reduce(msbLsb),
						z_peak_two_Hz: payload.slice(33, 35).reduce(msbLsb),
						z_peak_three_Hz: payload.slice(35, 37).reduce(msbLsb)
					};
				}
				// else{
				// 	parsed.data = {'error': 'Vibration mode error'}
