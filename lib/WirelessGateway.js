...
module.exports = class WirelessSensor{
	...
	parse(frame){
		...
		if(typeof this[type] == 'function'){
			...
			// #OTF
			var otf_devices = [4,12,21,23,26,32,33,39,44,45,48,52,53,54,55,56,58,74,76,78,79,80,81,82,84,88,89,90,91,97,98,101,102,103,105,106,107,108,109,110,111,112,114,117,118,119,120,121,122,123,124,180,181,202,217,211,270,519,520,521,531,535,536,537,538,539,540,541,542,543,1010,1011];
	...
	sensor_data(payload, frame){
		...
		// #OTF
		var otf_devices = [4,12,21,23,26,32,33,39,44,45,48,52,53,54,55,56,58,74,76,78,79,80,81,82,84,88,89,90,91,97,98,101,102,103,105,106,107,108,109,110,111,112,114,117,118,119,120,121,122,123,124,180,181,202,211,217,270,519,520,521,531,535,536,537,538,539,540,541,542,543,1010,1011];
		...
		if(payload.length == 179){
			...
			if(typeof this.sensor_types[parsed.sensor_type] == 'undefined'){
				...
				// #OTF
			}else if(parsed.sensor_type == 21 || parsed.sensor_type == 33 || parsed.sensor_type == 39 || parsed.sensor_type == 54 || parsed.sensor_type == 55 || parsed.sensor_type == 80 ||  parsed.sensor_type == 81 || parsed.sensor_type == 82 || parsed.sensor_type == 84 || parsed.sensor_type == 97 || parsed.sensor_type == 98 || parsed.sensor_type == 103 || parsed.sensor_type == 110 || parsed.sensor_type == 111 || parsed.sensor_type == 112 || parsed.sensor_type == 114 || parsed.sensor_type == 117 || parsed.sensor_type == 118 || parsed.sensor_type == 180 || parsed.sensor_type == 181 || parsed.sensor_type == 202 || parsed.sensor_type == 515 || parsed.sensor_type == 519 || parsed.sensor_type == 531 || parsed.sensor_type == 537 || parsed.sensor_type == 538 || parsed.sensor_type == 543){
	...
	var types = {
		...
		'118': {
			...
			parse: (payload, parsed, mac) => {
				if(payload[7] & 1){
					console.log('Error found');
					parsed.data = {error: 'Error found, Sensor Probe 1'};
					return parsed;
				}
				if(payload[7] & 2){
					console.log('Error found');
					parsed.data = {error: 'Error found, Sensor Probe 2'};
					return parsed;
				}
				return{
		...
		'119': {
			...
			parse: (d, payload) => {
				...
				if((payload[7] & 2) != 0){
					console.log('Error found');
					// parsed.data = {error: 'Error found, Acclerometer Probe may be unattached'};
					let error = {error: 'Error found, Acclerometer'};
					return error;
				}
				let report_type = "Regular";
				switch(d[17]){
