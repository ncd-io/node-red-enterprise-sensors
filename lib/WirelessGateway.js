// ERROR CODES
// 00801 - multi-packet stream corrupted/missing data
// 00802 - multi-packet stream timed out
// 00803 - Sensor probe unattached
			...
			parse: (payload, parsed, mac) => {
				...
				if(payload[8] === 1){
					...
					if(Object.hasOwn(globalDevices[deviceAddr], 'err_msg_counter_val') && globalDevices[deviceAddr].err_msg_counter_val != parsed.counter){
						console.log('MARK ZULU');
						delete globalDevices[deviceAddr].err_msg_counter_val;
					}

					// DELETE THIS AS IT WILL CAUSE ERRORS EVERY TIME
		...
		'82': {
			...
			parse: (payload, parsed, mac) => {
				...
				if(payload[8] === 1){
					...
					// data object exists and current packet already exists in that object or if data object exists and previous packet not in data
					if(Object.hasOwn(globalDevices, deviceAddr) && Object.hasOwn(globalDevices[deviceAddr], 'data') && current_packet in globalDevices[deviceAddr].data || Object.hasOwn(globalDevices, deviceAddr) && Object.hasOwn(globalDevices[deviceAddr], 'data') && !(((current_packet&127)-1) in globalDevices[deviceAddr].data)){
						// if(!Object.hasOwn(globalDevices[deviceAddr], 'err_msg_counter_val')){
						//	 globalDevices[deviceAddr].err_msg_counter_val = parsed.counter;
						// }
						// If Data exists that means we're mid packet and an error occured.
					...
					if(current_packet == 1){
						...
						globalDevices[deviceAddr].stream_timeout = setTimeout(() => {
							...
							parsed.sensor_data = {
								'error': {
									code: 00802,
									text: 'Multi-Packet Stream Timed Out'
								}
							}
							parent._emitter.emit('ncd_error', parsed);
							parent._emitter.emit('ncd_error-'+parsed.sensor_type, parsed);
						...
						globalDevices[deviceAddr].data[current_packet] = payload.slice(sdata_start);
					}else if(Object.hasOwn(globalDevices, deviceAddr) && Object.hasOwn(globalDevices[deviceAddr], 'data')){
						// Not first packet and no error detected, append to data array
						globalDevices[deviceAddr].data[current_packet] = payload.slice(sdata_start);
					}

					if(Object.hasOwn(globalDevices, deviceAddr) && Object.hasOwn(globalDevices[deviceAddr], 'data') && Object.keys(globalDevices[deviceAddr].data).length == expected_packets && !Object.hasOwn(globalDevices[deviceAddr], 'err_msg_counter_val')){
						console.log('+_+_+_+_+_+_+_+_+');
						var raw_data = new Array();
						for(const packet in globalDevices[deviceAddr].data){
							raw_data = raw_data.concat(globalDevices[deviceAddr].data[packet]);
