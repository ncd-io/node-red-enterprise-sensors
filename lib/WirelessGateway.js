...
module.exports = class WirelessSensor{
	...
	parse(frame){
		...
		if(typeof this[type] == 'function'){
			...
			// #OTF
			var otf_devices = [4,12,21,23,26,32,33,39,44,45,48,52,53,56,58,74,76,78,79,80,81,82,84,88,89,90,91,97,98,101,102,103,105,106,107,108,109,110,111,112,114,117,118,120,121,122,123,124,180,181,202,217,211,270,519,520,521,531,535,537,538,539,540,541,1010,1011];
	...
	sensor_data(payload, frame){
		...
		// #OTF
		var otf_devices = [4,12,21,23,26,32,33,39,44,45,48,52,53,56,58,74,76,78,79,80,81,82,84,88,89,90,91,97,98,101,102,103,105,106,107,108,109,110,111,112,114,117,118,120,121,122,123,124,180,181,202,211,217,270,519,520,521,531,535,537,538,539,540,541,1010,1011];
...
function sensor_types(parent){
	var types = {
		...
		'124': {
			...
			parse: (d) => {
				return {
					volume_flow: signInt(d.slice(0, 3).reduce(msbLsb), 32) / 100,
					...
					total_flow_1: signInt(d.slice(16, 24).reduce(msbLsb),32) / 100,
					total_flow_2: signInt(d.slice(24, 32).reduce(msbLsb),32) / 100,
					total_flow_3: signInt(d.slice(32, 40).reduce(msbLsb),32) / 100,
			...
			'parse_fly': (frame) => {
				return {
					'firmware': frame[2],
					'flow_unit': frame[13],
					'pressure_unit': frame[14],
					'conductivity_unit': frame[15],
					'totalizer_1_unit': frame[16],
					'totalizer_2_unit': frame[17],
					'totalizer_3_unit': frame[18],
					'hardware_id': frame.slice(19, 22),
					'sample_rate': frame.slice(22, 26).reduce(msbLsb),
					'tx_life_counter': frame.slice(26, 30).reduce(msbLsb),
					'machine_values': {
						'firmware': frame[2],
						'flow_unit': frame[13],
						'pressure_unit': frame[14],
						...
						'totalizer_3_unit': frame[18],
						'hardware_id': frame.slice(19, 22),
						'sample_rate': frame.slice(22, 26),
						'tx_life_counter': frame.slice(26, 30),
					}
				}
			}
		},
