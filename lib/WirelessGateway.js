...
function sensor_types(parent){
	var types = {
		...
		'97': {
			...
			parse: (d) => {
				if(d[0] === 0){ // processed mode
					return {
						mode: d[0],
						rms_voltage: d.slice(1, 5).reduce(msbLsb) * 1000,
						freq_1: d.slice(5, 7).reduce(msbLsb) * 3,
						freq_2: d.slice(7, 9).reduce(msbLsb) * 3,
						freq_3: d.slice(9, 11).reduce(msbLsb) * 3
					};
				}
				else if (d[0] === 1) { // raw
					return {
						...
						sensor_index: d[1],
						gain_db: d[2],
						total_packets: d.slice(3, 5).reduce(msbLsb),
						current_packet_index: d.slice(5, 7).reduce(msbLsb),
						payload: d.slice(7)
					};
				}
			}
		...
		'98': {
			...
			parse: (d) => {
				if(d[0] === 0){ // processed mode
					return {
						mode: d[0],
						c1_rms_voltage: d.slice(1, 5).reduce(msbLsb) * 1000,
						c1_freq_1: d.slice(5, 7).reduce(msbLsb) * 3,
						c1_freq_2: d.slice(7, 9).reduce(msbLsb) * 3,
						c1_freq_3: d.slice(9, 11).reduce(msbLsb) * 3,
						c2_rms_voltage: d.slice(11, 15).reduce(msbLsb) * 1000,
						c2_freq_1: d.slice(15, 17).reduce(msbLsb) * 3,
						c2_freq_2: d.slice(17, 19).reduce(msbLsb) * 3,
						c2_freq_3: d.slice(19, 21).reduce(msbLsb) * 3
					};
				}
				else if (d[0] === 1) { // raw
					return {
