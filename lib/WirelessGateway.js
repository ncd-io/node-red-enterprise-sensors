		...
		'536': {
			...
			parse: (payload, parsed) => {
				return {
					...
					s1_saturation_percent: payload.slice(9, 13).reduce(msbLsb)/1000,
					s2_saturation_percent: payload.slice(13, 17).reduce(msbLsb)/1000,
					s3_saturation_percent: payload.slice(17, 21).reduce(msbLsb)/1000,
					s4_saturation_percent: payload.slice(21, 25).reduce(msbLsb)/1000,
					s1_oxigen_ppm: payload.slice(25, 29).reduce(msbLsb)/1000,
					s2_oxigen_ppm: payload.slice(29, 33).reduce(msbLsb)/1000,
					s3_oxigen_ppm: payload.slice(33, 37).reduce(msbLsb)/1000,
					s4_oxigen_ppm: payload.slice(37, 41).reduce(msbLsb)/1000,
					s1_oxigen_mg_l: payload.slice(41, 45).reduce(msbLsb)/1000,
					s2_oxigen_mg_l: payload.slice(45, 49).reduce(msbLsb)/1000,
					s3_oxigen_mg_l: payload.slice(49, 53).reduce(msbLsb)/1000,
					s4_oxigen_mg_l: payload.slice(53, 57).reduce(msbLsb)/1000,
					s1_flow_rate: payload.slice(57, 61).reduce(msbLsb)/1000,
					s2_flow_rate: payload.slice(61, 65).reduce(msbLsb)/1000,
					s3_flow_rate: payload.slice(65, 69).reduce(msbLsb)/1000,
					s4_flow_rate: payload.slice(69, 73).reduce(msbLsb)/1000,
