...
module.exports = class WirelessSensor{
	constructor(digi){
		...
		function remoteATReceiver(frame){
			console.log('remoteATReceiver');
			that.remote_at_receiver(frame);
		}
		...
		this.digi.on('at_command_response', remoteATReceiver)
		this.on('close', () => {
			...
			this.digi._emitter.removeListener('at_command_response',remoteATReceiver);
		});
	}
	...
	remote_at_receiver(frame){
		console.log("remote_at_receiver: "+frame);
		// var msg_obj = {};
		// msg_obj.source_address = frame.source_mac;
		// msg_obj.destination = toMac(frame.data.slice(0,8));
		// msg_obj.payload_size = msbLsb(frame.data[8],frame.data[9]);
		...
		return new Promise((fulfill, reject) => {
			that.queue.add(() => {
				return new Promise((f, r) => {
					...
					function send(){
						that.send.remote_at_command(mac2bytes(sensor_mac), parameter, value, opts, true).then(function(frame){
							if(frame.delivery_status == 'Success'){
								pass(frame);
							}else{
								tO = setTimeout(() => {
