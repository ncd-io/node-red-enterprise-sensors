...
module.exports = class WirelessSensor{
	...
	parse(frame){
		...
		if(typeof this[type] == 'function'){
			...
			// #OTF
			var otf_devices = [4,12,21,23,26,32,33,39,44,45,48,52,53,56,58,74,76,78,79,80,81,82,84,88,89,90,91,97,98,101,102,103,105,106,107,108,109,110,111,112,114,117,118,119,120,121,122,123,124,180,181,202,217,211,270,519,520,521,531,535,536,537,538,539,540,541,542,1010,1011];
			...
			if(mode === 'FLY' && frame.data.length > 12 && typeof this.sensor_types[data.sensor_type].parse_fly == 'function'){
				...
				if(data.data.length){
					if(data.sensor_type == 110 || data.sensor_type == 111 || data.sensor_type == 112 || data.sensor_type == 114){
	...
	sensor_data(payload, frame){
		...
		if(sensor_type == 110 || sensor_type == 111 || sensor_type == 112 || sensor_type == 114){
			const vMin = 2.5;
			const vMax = 3.3;
		...
		// #OTF
		var otf_devices = [4,12,21,23,26,32,33,39,44,45,48,52,53,56,58,74,76,78,79,80,81,82,84,88,89,90,91,97,98,101,102,103,105,106,107,108,109,110,111,112,114,117,118,119,120,121,122,123,124,180,181,202,211,217,270,519,520,521,531,535,536,537,538,539,540,541,542,1010,1011];
			...
			parse: (d, payload) => {
				...
				if ((payload[7] & 2) != 0) {
					return {
						...
						'accelero_sample_1':{
							x:[signInt(d.slice(21, 23).reduce(msbLsb),16)/100],
							y:[signInt(d.slice(23, 25).reduce(msbLsb),16)/100],
							z:[signInt(d.slice(25, 27).reduce(msbLsb),16)/100]
						},
						'accelero_sample_2':{
							x:[signInt(d.slice(27, 29).reduce(msbLsb),16)/100],
							y:[signInt(d.slice(29, 31).reduce(msbLsb),16)/100],
							z:[signInt(d.slice(31, 33).reduce(msbLsb),16)/100]
						},
						'accelero_sample_3':{
							x:[signInt(d.slice(33, 35).reduce(msbLsb),16)/100],
							y:[signInt(d.slice(35, 37).reduce(msbLsb),16)/100],
							z:[signInt(d.slice(37, 39).reduce(msbLsb),16)/100]
						},
						'accelero_sample_4':{
							x:[signInt(d.slice(39, 41).reduce(msbLsb),16)/100],
							y:[signInt(d.slice(41, 43).reduce(msbLsb),16)/100],
							z:[signInt(d.slice(43, 45).reduce(msbLsb),16)/100]
						},
						'accelero_sample_5':{
							x:[signInt(d.slice(45, 47).reduce(msbLsb),16)/100],
							y:[signInt(d.slice(47, 49).reduce(msbLsb),16)/100],
							z:[signInt(d.slice(49, 51).reduce(msbLsb),16)/100]
						},
						'accelero_sample_6':{
							x:[signInt(d.slice(51, 53).reduce(msbLsb),16)/100],
							y:[signInt(d.slice(53, 55).reduce(msbLsb),16)/100],
							z:[signInt(d.slice(55, 57).reduce(msbLsb),16)/100]
						},
						'accelero_sample_7':{
							x:[signInt(d.slice(57, 59).reduce(msbLsb),16)/100],
							y:[signInt(d.slice(59, 61).reduce(msbLsb),16)/100],
							z:[signInt(d.slice(61, 63).reduce(msbLsb),16)/100]
						},
						'accelero_sample_8':{
							x:[signInt(d.slice(63, 65).reduce(msbLsb),16)/100],
							y:[signInt(d.slice(65, 67).reduce(msbLsb),16)/100],
							z:[signInt(d.slice(67, 69).reduce(msbLsb),16)/100]
						},
						'accelero_sample_9':{
							x:[signInt(d.slice(69, 71).reduce(msbLsb),16)/100],
							y:[signInt(d.slice(71, 73).reduce(msbLsb),16)/100],
							z:[signInt(d.slice(73, 75).reduce(msbLsb),16)/100]
						},
						'accelero_sample_10':{
							x:[signInt(d.slice(75, 77).reduce(msbLsb),16)/100],
							y:[signInt(d.slice(77, 79).reduce(msbLsb),16)/100],
							z:[signInt(d.slice(79, 81).reduce(msbLsb),16)/100]
						},
						'temperature': d.slice(81, 83).reduce(msbLsb)/100
					};
				}else{
					return {
						raw_data: "Disabled",
