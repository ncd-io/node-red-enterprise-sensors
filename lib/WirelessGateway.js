...
module.exports = class WirelessSensor{
	...
	parse(frame){
		...
		if(typeof this[type] == 'function'){
			...
			// #OTF
			var otf_devices = [4,12,21,23,26,32,33,39,44,45,48,52,53,56,58,74,76,78,79,80,81,82,84,88,89,90,91,97,98,101,102,103,105,106,107,108,109,110,111,112,114,117,118,120,121,122,123,124,180,181,202,217,211,270,519,520,521,531,535,536,537,538,539,540,541,1010,1011];
	...
	sensor_data(payload, frame){
		...
		// #OTF
		var otf_devices = [4,12,21,23,26,32,33,39,44,45,48,52,53,56,58,74,76,78,79,80,81,82,84,88,89,90,91,97,98,101,102,103,105,106,107,108,109,110,111,112,114,117,118,120,121,122,123,124,180,181,202,211,217,270,519,520,521,531,535,536,537,538,539,540,541,1010,1011];
	...
	var types = {
		...
		'114': {
			...
			'parse_fly': (frame) => {
				...
				}else if(frame[2] > 5){ // for Firmware v6 and above
					frame_data.motion_to_delay = frame[41] * 50;
					return {
						'firmware': frame[2],
						'destination_address': toMac(frame.slice(12, 16)),
		...
		'536': {
			...
			parse: (payload, parsed) => {
				return {
					...
					s1_oxygen_temp: payload.slice(1, 3).reduce(msbLsb),
					s2_oxygen_temp: payload.slice(3, 5).reduce(msbLsb),
					s3_oxygen_temp: payload.slice(5, 7).reduce(msbLsb),
					s4_oxygen_temp: payload.slice(7, 9).reduce(msbLsb),
					s1_saturation_percent: payload.slice(9, 13).reduce(msbLsb),
					s2_saturation_percent: payload.slice(13, 17).reduce(msbLsb),
					s3_saturation_percent: payload.slice(17, 21).reduce(msbLsb),
					s4_saturation_percent: payload.slice(21, 25).reduce(msbLsb),
					s1_oxigen_ppm: payload.slice(25, 29).reduce(msbLsb),
					s2_oxigen_ppm: payload.slice(29, 33).reduce(msbLsb),
					s3_oxigen_ppm: payload.slice(33, 37).reduce(msbLsb),
					s4_oxigen_ppm: payload.slice(37, 41).reduce(msbLsb),
					s1_oxigen_mg_l: payload.slice(41, 45).reduce(msbLsb),
					s2_oxigen_mg_l: payload.slice(45, 49).reduce(msbLsb),
					s3_oxigen_mg_l: payload.slice(49, 53).reduce(msbLsb),
					s4_oxigen_mg_l: payload.slice(53, 57).reduce(msbLsb),
					s1_flow_rate: payload.slice(57, 61).reduce(msbLsb),
					s2_flow_rate: payload.slice(61, 65).reduce(msbLsb),
					s3_flow_rate: payload.slice(65, 69).reduce(msbLsb),
					s4_flow_rate: payload.slice(69, 73).reduce(msbLsb),
