	...
	var types = {
		...
		'108': {
			...
			'parse_fly': (frame) => {
				...
				} else if(frame[2] > 15){ // Screen version
					let reset_mode = "Disabled";
					switch(frame[38]){
						case 0:
							reset_mode = "Disabled";
		...
		'111': {
			...
			parse: (payload, parsed, mac) => {
				...
				if(payload[8] === 1){
					...
					console.log('expected_packets:', expected_packets);
					console.log(globalDevices[deviceAddr].hasOwnProperty(probe));
					if(globalDevices[deviceAddr].hasOwnProperty(probe) || expected_packets == 1){
						...
						console.log('Initializing new probe stream');
						// clear stream object & timeout
						parent.sensor_types[111].utils.clear_globalDevices_stream(deviceAddr, probe);
					...
					if(current_packet == expected_packets){
						console.log('Final packet received');
						// concatenate stream
			...
			'utils': {
				...
				'init_globalDevices_stream'(deviceAddr, payload, expected_packets, parsed, msg_type, probe){
					...
					// console.log(globalDevices[deviceAddr]);
				},
