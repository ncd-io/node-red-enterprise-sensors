...
module.exports = class WirelessSensor{
	...
	parse(frame){
		...
		if(typeof this[type] == 'function'){
			...
			// #OTF
			var otf_devices = [4,12,21,23,26,29,30,32,33,39,44,45,48,52,53,54,55,56,58,74,76,78,79,80,81,82,84,88,89,90,91,97,98,99,101,102,103,105,106,107,108,109,110,111,112,114,117,118,119,120,121,122,123,124,180,181,202,217,211,270,519,520,521,531,535,536,537,538,539,540,541,542,543,545,554,1010,1011];
	...
	sensor_data(payload, frame){
		...
		// #OTF
		var otf_devices = [4,12,21,23,26,29,30,32,33,39,44,45,48,52,53,54,55,56,58,74,76,78,79,80,81,82,84,88,89,90,91,97,98,99,101,102,103,105,106,107,108,109,110,111,112,114,117,118,119,120,121,122,123,124,180,181,202,211,217,270,519,520,521,531,535,536,537,538,539,540,541,542,543,545,554,1010,1011];
...
function sensor_types(parent){
	var types = {
		...
		'541': {
			...
			parse: (d) => {
				return {
					adc1: signInt(d.slice(0, 2).reduce(msbLsb), 16),
					adc2: signInt(d.slice(2, 4).reduce(msbLsb), 16),
					mA1: signInt(d.slice(4, 6).reduce(msbLsb), 16)/100,
					mA2: signInt(d.slice(6, 8).reduce(msbLsb), 16)/100,
					flow_1: signInt(d.slice(8, 12).reduce(msbLsb), 32)/100,
					flow_2:signInt(d.slice(12, 16).reduce(msbLsb), 32)/100
				};
			},
			'parse_fly': (frame) => {
				let firmware = frame[2];
		...
		'554': {
			...
			parse: (payload, parsed, mac) => {
				let status = [];
				if(parsed[8] & 1) status.push('fatal');
				if(parsed[8] & 2) status.push('offset_reg');
				if(parsed[8] & 4) status.push('algorithm');
				if(parsed[8] & 8) status.push('output');
				if(parsed[8] & 16) status.push('self_diagnostic');
				if(parsed[8] & 32) status.push('out_of_range');
				if(parsed[8] & 64) status.push('memory');
				if(parsed[8] == 0) status = 'valid_data'
				let res = {
					status: status,
					co2_ppm: signInt(parsed.slice(9, 11).reduce(msbLsb), 16)
				};
				if(parsed[7] == 1) res.co2_ppm = 'invalid_data';
				return res;
			},
			'parse_fly': (frame) => {
				return {
