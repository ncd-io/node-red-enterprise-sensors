...
module.exports = class WirelessSensor{
	...
	parse(frame){
		...
		if(typeof this[type] == 'function'){
			...
			// #OTF
			var otf_devices = [4,12,21,23,26,29,30,32,33,35,39,44,45,48,52,53,54,55,56,58,74,76,78,79,80,81,82,84,88,89,90,91,97,98,99,101,102,103,105,106,107,108,109,110,111,112,114,117,118,119,120,121,122,123,124,180,181,202,217,211,270,519,520,521,531,535,536,537,538,539,540,541,542,543,545,554,1010,1011];
	...
	sensor_data(payload, frame){
		...
		// #OTF
		var otf_devices = [4,12,21,23,26,29,30,32,33,35,39,44,45,48,52,53,54,55,56,58,74,76,78,79,80,81,82,84,88,89,90,91,97,98,99,101,102,103,105,106,107,108,109,110,111,112,114,117,118,119,120,121,122,123,124,180,181,202,211,217,270,519,520,521,531,535,536,537,538,539,540,541,542,543,545,554,1010,1011];
	...
	var types = {
		...
		'84': {
				...
				if(globalDevices.hasOwnProperty(deviceAddr) || expected_packets == 1){
						if(expected_packets != 1){
							...
							if(current_packet == 1 || (globalDevices[deviceAddr].last_packet_counter > current_packet)){
								...
								globalDevices[deviceAddr].en_axis = en_axis;
			...
			'utils': {
				...
				'concat_fft_data': (deviceAddr, mode) => {
					var raw_data = new Array();
					for(const packet in globalDevices[deviceAddr].data){
						raw_data = raw_data.concat(globalDevices[deviceAddr].data[packet]);
					}
		...
		'554': {
			...
			parse: (payload, parsed, mac) => {
				...
				let res = {
					status: status,
					co2_ppm: signInt(payload.slice(9, 11).reduce(msbLsb), 16)
				};
