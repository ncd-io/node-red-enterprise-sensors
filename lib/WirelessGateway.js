	...
	var types = {
		...
		'97': {
			...
			parse: (payload, parsed, mac) => {
				if (payload[8] === 1) { // raw
					var mode = payload[8];
					var sensor_index = payload[9];
					var gain_db = payload[10];
					var deviceAddr = mac;
					var expected_packets = payload.slice(11, 13).reduce(msbLsb);
					var current_packet = payload.slice(13, 15).reduce(msbLsb);
					var sdata_start = 15;
					...
					return {
						mode: payload[8],
						rms_voltage: payload.slice(9, 11).reduce(msbLsb) * 1000,
						freq_1: payload.slice(11, 13).reduce(msbLsb) * 3,
						freq_2: payload.slice(13, 15).reduce(msbLsb) * 3,
						freq_3: payload.slice(15, 17).reduce(msbLsb) * 3,
					};
				}
		...
		'98': {
			...
			parse: (payload, parsed, mac) => {
				if (payload[8] === 1) { // raw
					...
					return {
						mode: payload[8],
						c1_rms_voltage: payload.slice(9, 11).reduce(msbLsb) * 1000,
						c1_freq_1: payload.slice(11, 13).reduce(msbLsb) * 3,
						c1_freq_2: payload.slice(13, 15).reduce(msbLsb) * 3,
						c1_freq_3: payload.slice(15, 17).reduce(msbLsb) * 3,
						c2_rms_voltage: payload.slice(17, 21).reduce(msbLsb) * 1000,
						c2_freq_1: payload.slice(21, 23).reduce(msbLsb) * 3,
						c2_freq_2: payload.slice(23, 25).reduce(msbLsb) * 3,
						c2_freq_3: payload.slice(25, 27).reduce(msbLsb) * 3
					};
				}
			},
			'parse_fly': (frame) => {
				return {
					...
					'raw_on_request_timeout': frame[13] + " Sec",
					'fly_rate': frame.slice(14, 16).reduce(msbLsb),
					'sensor_1_gain': frame[16],
					'sensor_1_boot_time': frame[17],
					'sensor_2_gain': frame[18],
					'sensor_2_boot_time': frame[19],
					'hardware_id_1': frame[20],
					'hardware_id_2': frame[21],
					'hardware_id_3': frame[22],
					'rate': frame.slice(23, 27).reduce(msbLsb) + " Sec",
			...
			parse: (payload, parsed, mac) => {
				...
				if(payload[8] === 1){
					...
					return {
						...
						z_peak_two_Hz: payload.slice(50, 52).reduce(msbLsb),
						z_peak_three_Hz: payload.slice(52, 54).reduce(msbLsb),
						rpm: signInt(payload.slice(54, 56).reduce(msbLsb), 16) / 10
					};
				}
				// else{
				// 	parsed.data = {'error': 'Vibration mode error'}
			...
			parse: (payload, parsed, mac) => {
				...
				if(payload[8] === 1){
					...
					return {
						...
						z1_peak_two_Hz: payload.slice(50, 52).reduce(msbLsb),
						z1_peak_three_Hz: payload.slice(52, 54).reduce(msbLsb),
						rpm_1: signInt(payload.slice(54, 56).reduce(msbLsb), 16) / 10,
						...
						z2_peak_two_Hz: payload.slice(95, 97).reduce(msbLsb),
						z2_peak_three_Hz: payload.slice(97, 99).reduce(msbLsb),
						rpm_2: signInt(payload.slice(99, 101).reduce(msbLsb), 16) / 10
					};
				}
				// else{
				// 	parsed.data = {'error': 'Vibration mode error'}
