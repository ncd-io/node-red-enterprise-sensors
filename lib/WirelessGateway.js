...
module.exports = class WirelessSensor{
	...
	parse(frame){
		...
		if(typeof this[type] == 'function'){
			...
			// #OTF
			var otf_devices = [4,12,21,23,26,32,33,39,44,45,48,52,53,56,58,74,76,78,79,80,81,82,84,88,89,90,91,97,98,101,102,103,105,106,107,108,109,110,111,112,114,118,119,120,180,181,202,217,519,520,521,531,535,537,538,539,540,1010,1011];
	...
	config_set_wood_type_119(sensor_mac, value){
		console.log('config_set_wood_type_119');
		...
		'103': {
			...
			parse: (payload, parsed, mac) => {
				if(payload[9] === 0){ // mode raw
					...
					var expected_packets =  payload.slice(18, 20).reduce(msbLsb);
					var current_packet = payload.slice(20, 22).reduce(msbLsb);
					var data_start = 22;
		...
		'119':{
			...
			parse: (d) => {
				return {
					...
					wood_moisture: d.slice(5, 9).reduce(msbLsb) / 100,
					wood_resistance: d.slice(9, 13).reduce(msbLsb) / 100
				};
			},
			'parse_fly': (frame) => {
				return {
