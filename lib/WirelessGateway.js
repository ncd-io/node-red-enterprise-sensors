...
module.exports = class WirelessSensor{
	...
	// opts.packets     test packets per run   (1-65535, default 200)
	// opts.packetSize  payload bytes per packet (1-65535, default 200)
			...
			that.queue.add(() => {
				return new Promise((f, r) => {
					...
					function send(){
						...
						// Both are big-endian uint16 on the wire, so the full 1-65535 range is
						// supported. Defaults stay at 200/200, which is what this always sent.
						var size = clamp_uint16(opts && opts.packetSize, 200);
						var count = clamp_uint16(opts && opts.packets, 200);
						var payload_size = [size >> 8, size & 255];
						var iterations = [count >> 8, count & 255];
						...
						// Frame options for explicit_addressing_command - deliberately NOT named
						// `opts`: a `var opts` here shadows link_test's own `opts` parameter for
						// the whole of send(), which is why the caller's packet count and size
...
// Coerce a caller-supplied value into a uint16, falling back to `dflt` for
// anything that isn't a finite number. Clamps rather than throwing: these come
// from user configuration, and a bad value should not reach the radio.
function clamp_uint16(v, dflt){
	// null and '' both coerce to 0, which is finite, so they have to be
	// rejected explicitly or an omitted option silently becomes 1.
