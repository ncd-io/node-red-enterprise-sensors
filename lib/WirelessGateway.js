...
function sensor_types(parent){
	var types = {
		...
		'119': {
			name: 'Wireless Machine Run Timer Meter',
			parse: (d, payload) => {
				let firmware = payload[1];
				if(payload[7] & 2 != 0){
					console.log('Error found');
				...
				return {
					...
					'accelero_sample_1':{
						x:[d.slice(21, 23).reduce(msbLsb)],
						y:[d.slice(23, 25).reduce(msbLsb)],
						z:[d.slice(25, 27).reduce(msbLsb)]
					},
					'accelero_sample_2':{
						x:[d.slice(27, 29).reduce(msbLsb)],
						y:[d.slice(29, 31).reduce(msbLsb)],
						z:[d.slice(31, 33).reduce(msbLsb)]
					},
					'accelero_sample_3':{
						x:[d.slice(33, 35).reduce(msbLsb)],
						y:[d.slice(35, 37).reduce(msbLsb)],
						z:[d.slice(37, 39).reduce(msbLsb)]
					},
					'accelero_sample_4':{
						x:[d.slice(39, 41).reduce(msbLsb)],
						y:[d.slice(41, 43).reduce(msbLsb)],
						z:[d.slice(43, 45).reduce(msbLsb)]
					},
					'accelero_sample_5':{
						x:[d.slice(45, 47).reduce(msbLsb)],
						y:[d.slice(47, 49).reduce(msbLsb)],
						z:[d.slice(49, 51).reduce(msbLsb)]
					},
					'accelero_sample_6':{
						x:[d.slice(51, 53).reduce(msbLsb)],
						y:[d.slice(53, 55).reduce(msbLsb)],
						z:[d.slice(55, 57).reduce(msbLsb)]
					},
					'accelero_sample_7':{
						x:[d.slice(57, 59).reduce(msbLsb)],
						y:[d.slice(59, 61).reduce(msbLsb)],
						z:[d.slice(61, 63).reduce(msbLsb)]
					},
					'accelero_sample_8':{
						x:[d.slice(63, 65).reduce(msbLsb)],
						y:[d.slice(65, 67).reduce(msbLsb)],
						z:[d.slice(67, 69).reduce(msbLsb)]
					},
					'accelero_sample_9':{
						x:[d.slice(69, 71).reduce(msbLsb)],
						y:[d.slice(71, 73).reduce(msbLsb)],
						z:[d.slice(73, 75).reduce(msbLsb)]
					},
					'accelero_sample_10':{
						x:[d.slice(75, 77).reduce(msbLsb)],
						y:[d.slice(77, 79).reduce(msbLsb)],
						z:[d.slice(79, 81).reduce(msbLsb)]
					},
			...
			'parse_fly': (frame) => {
				...
				let acc_interrupt_type = 'OR';
				switch(frame[41]){
					case 0:
						acc_interrupt_type = 'OR';
						break;
					case 1:
						acc_interrupt_type = 'AND';
						break;
				}
				return {
					'firmware': frame[2],
					...
					'acc_int_mode': acc_interrupt_type,
