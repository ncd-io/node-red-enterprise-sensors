...
module.exports = class WirelessSensor{
	...
	parse(frame){
		...
		if(typeof this[type] == 'function'){
			...
			// #OTF
			var otf_devices = [23,26,39,45,48,52,58,74,76,78,79,80,81,82,84,88,89,90,91,101,102,105,106,107,108,110,111,112,114,180,181,519,520,521,531,535,537,538,539,540,1010,1011];
	...
	var types = {
		...
		'74': {
			...
			parse: (d) => {
				return {
					temperature: signInt(d.slice(0, 2).reduce(msbLsb), 16) *100,
					pressure: signInt(d.slice(2, 6).reduce(msbLsb), 16) * 100,
					humidity: signInt(d.slice(6, 10).reduce(msbLsb), 16) * 1000,
					resistance: signInt(d.slice(10, 14).reduce(msbLsb), 16),
					iaq: signInt(d.slice(14, 16).reduce(msbLsb), 16),
					co2_eqv: signInt(d.slice(16, 18).reduce(msbLsb), 16),
					breath_voc: signInt(d.slice(18, 22).reduce(msbLsb), 16) * 100,
					iaq_accuracy: signInt(d[22], 16)
				};
			}
		},
		'75': {
