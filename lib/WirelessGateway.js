...
module.exports = class WirelessSensor{
	...
	firmware_send_v13(sensor_mac, data, opts, cmd_timeout = 7000){
		var that = this;
		return new Promise((fulfill, reject) => {
			that.queue.add(() => {
				return new Promise((f, r) => {
					...
					function startTimeout(){
						tout = setTimeout(() => {
							...
							// if(retry_count < max_retries) {
							// 	retry_count++;
							// 	console.log(`Retrying (attempt ${retry_count}) due to timeout`);
							// 	clearTimeout(tout);
							// 	startTimeout(retry_count);
							// 	attempt_transmission();
							// } else{
			...
			parse: (payload, parsed, mac) => {
				...
				if(payload[8] === 1){
					...
					return {
						...
						x_ppv: payload.slice(14, 16).reduce(msbLsb) / 100,
						x_max_velocity: payload.slice(16, 18).reduce(msbLsb) / 100,
						x_rms_velocity: payload.slice(18, 20).reduce(msbLsb) / 100,
						x_peak_one_Hz: payload.slice(20, 22).reduce(msbLsb),
						x_peak_two_Hz: payload.slice(22, 24).reduce(msbLsb),
						x_peak_three_Hz: payload.slice(24, 26).reduce(msbLsb),

						y_ppv: payload.slice(26, 28).reduce(msbLsb) / 100,
						y_max_velocity: payload.slice(28, 30).reduce(msbLsb) / 100,
						y_rms_velocity: payload.slice(30, 32).reduce(msbLsb) / 100,
						y_peak_one_Hz: payload.slice(32, 34).reduce(msbLsb),
						y_peak_two_Hz: payload.slice(34, 36).reduce(msbLsb),
						y_peak_three_Hz: payload.slice(36, 38).reduce(msbLsb),

						z_ppv: payload.slice(38, 40).reduce(msbLsb) / 100,
						z_max_velocity: payload.slice(40, 42).reduce(msbLsb) / 100,
						z_rms_velocity: payload.slice(42, 44).reduce(msbLsb) / 100,
						z_peak_one_Hz: payload.slice(44, 46).reduce(msbLsb),
						z_peak_two_Hz: payload.slice(46, 48).reduce(msbLsb),
						z_peak_three_Hz: payload.slice(48, 50).reduce(msbLsb)
					};
				}
				// else{
				// 	parsed.data = {'error': 'Vibration mode error'}
